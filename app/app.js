(() => {
  ...
  const state = { listening:false, expressive:false, round:0, ambiguityIndex:0, noneCount:0, known:[], fragment:'', confirmed:false, message:'' };
  const ambiguities = [
  ...
  function beginFragment(fragment) {
    if (!fragment) return showFallback(); state.fragment = fragment; state.round = 0; state.noneCount = 0; state.ambiguityIndex = 0; state.known = []; $('flowPanel').hidden = false; if (modeA) return showDirectCandidates(); nextClarification();
  }
  ...
  function showClarification(item, choices) { const started = performance.now(); state.round += 1; logLatency('llm: clarification choices', started, item.key); renderFlow(`<p class="eyebrow">${state.round} / 2 回目</p><h2>${item.question}</h2><p class="flow-note">短く選んでください。ことば：${escapeHtml(state.fragment)}</p>`, choices, (choice) => { if (choice === 'どれも違います') return noneOfThese(); state.known.push(item.key); state.ambiguityIndex = ambiguities.indexOf(item) + 1; if (state.round >= 2 || item.key === 'content') { state.message = buildMessage(); showConfirm(); } else nextClarification(); }, true); }
  function noneOfThese() { if (state.noneCount === 0) { state.noneCount = 1; showClarification(state.activeAmbiguity, state.activeAmbiguity.alternatives); } else showFallback(); }
