(() => {
  ...
  function beginFragment(fragment) {
    if (!fragment) return showFallback(); state.fragment = fragment; state.round = 0; state.noneCounts = {}; state.ambiguityIndex = 0; state.known = []; state.answers = {}; $('flowPanel').hidden = false; if (modeA) return showDirectCandidates(); nextClarification();
  }
