...
- [x] T068 [US1] Fold the old open-question answer-candidate branch into `op=simplify`'s optional `options` field in `worker/prompts/simplify.txt` and `app/pipelines/receptive.js`; delete the separate path from `app/app.js`
  ...
  - Historical deployment note: at T068 the live Worker still predated T063 and returned the legacy `{choices}` shape. **Resolved 2026-09-25** by manual redeploy of `aphasia-ai-proxy` (Version ID `f9447876-b368-4715-913c-58580ce86084`); live smoke checks confirmed current `op=simplify` and `op=hypotheses` contracts.
